...
def convertToWebp(old_path,paths_dict,tmppath):
    tmpname = hashlib.md5(old_path.encode()).hexdigest()
    path = Path(old_path)
    tpath = Path(os.path.join(tmppath,f"{tmpname}.tmp{Path(old_path).suffix}"))    
    newpath = Path(os.path.join(tmppath,f"{tmpname}.webp"))  
    if(newpath.exists() and os.stat(newpath).st_size != 0):
    ...
    cwebpcall(tpath,newpath)
    os.remove(tpath)
    paths_dict[old_path] = newpath
    return newpath

def cwebpcall( path, newpath):
    """ convert image from path to newpath webp. """
    cmd = ["cwebp","-lossless", path,"-o", newpath]
    subprocess_call(cmd)

def convertToPng(old_path,paths_dict,tmppath):
    tmpname = hashlib.md5(old_path.encode()).hexdigest()
    path = Path(old_path)
    tpath = Path(os.path.join(tmppath,f"{tmpname}.tmp.webp"))    
    newpath = Path(os.path.join(tmppath,f"{tmpname}.png"))  
    if(newpath.exists() and os.stat(newpath).st_size != 0):
        if(os.path.exists(tpath)):
            os.remove(tpath)
        paths_dict[old_path] = newpath
    ...
    newpath.parent.mkdir(exist_ok=True, parents=True)
    shutil.copy(path,tpath)
    dwebpcall(tpath,newpath)
    os.remove(tpath)
    paths_dict[old_path] = newpath
    return newpath

def dwebpcall( path, newpath):
    """ convert image from path to newpath png. """
    cmd = ["dwebp", path,"-o", newpath]
    subprocess_call(cmd)
...
def convertQueue(q,mode,paths_dict,tmppath):
    global gui,oldsize,newsize,pause
    while(not q.empty() and not stop.is_set()):
        path = q.get()
        if mode == "encode":
            newpath = convertToWebp(path,paths_dict,tmppath)
        elif mode == 'decode':
            newpath = convertToPng(path,paths_dict,tmppath)
        with data_lock:
            try:
...
def convert(basedir,images,mode,globaltmp):
    paths_dict = {}
    ...
    if(globaltmp):
        ...
        tmppath = os.path.join(basedir,"tmp_convert")
        
    num_theads = 3
    workers = []
    for i in range(num_theads):
        worker = Thread(target=convertQueue, args=(proxyq,mode,paths_dict,tmppath),name="worker {}".format(i),daemon=True)
    ...
    if os.path.isdir(tmppath):
        for file in images:
            if(os.path.exists(paths_dict[file]) and os.stat(paths_dict[file]).st_size != 0):
                os.remove(file)
                shutil.move(paths_dict[file], Path(file).with_suffix(paths_dict[file].suffix))
            else:
...
def get_images(path,mode):
    ...
    for root, dirs, files in os.walk(path):
        ...
        for file in sorted(files,key=natural_keys):
            ...
            if(mode == 'encode'):
                if guess_type and (guess_type.startswith('image/png') or guess_type.startswith('image/bmp')):
                    images.append(filepath)
            elif(mode == 'decode'):
                if guess_type and guess_type.startswith('image/webp'):
                    images.append(filepath)
        break
    return images

def filter_files(files,mode):
    images = []
    for file in sorted(files,key=natural_keys):
...
def main(files,dirs,globaltmp=False,mode="encode"):
    global gui
    if(len(dirs) == 1 and len(files) == 0):
        files = get_images(dirs[0],mode)
        dirs.clear()
    ...
    if(files):
        images = filter_files(files,mode)
        gui.progress.reset(gui.files_task_id,total=len(images))
        convert(os.path.dirname(files[0]),images,mode,globaltmp)
        if(total > 1):
            gui.progress.update(gui.directories_task_id,advance=1,refresh=True)
    if(dirs):
        for current_dir in dirs:
            gui.current_directory = Text(f"{os.path.basename(current_dir)}",style="yellow2")
            images = get_images(current_dir,mode)
            gui.progress.reset(gui.files_task_id, total=len(images))
            convert(current_dir,images,mode,globaltmp)
            gui.progress.update(gui.directories_task_id,advance=1,refresh=True)
...
def convertwebp():
    ...
    group.add_argument('-e','--encode', action='store_const', dest='mode', const='encode' , help="png to webp")
    group.add_argument('-d','--decode', action='store_const', dest='mode', const='decode' , help="webp to png")
    ...
    mainthread = Thread(name="mainthread",target = main,args=(files,dirs,args.globaltmpfolder,args.mode))
    mainthread.start()
    gui.start()
    def toggle_pause():
        global pause,gui
